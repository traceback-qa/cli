...
const SHIM_PORT = 9001;
const API_HOST = '127.0.0.1';
const API_PORT = 8010;
const SESSION_FILE = '/tmp/relay_session_flutter.json';
const VM_PATH_FILE = '/tmp/vm_service_path.txt';
...
const server = http.createServer((req, res) => {
  ...
  const preq = http.request(
    { host: API_HOST, port: API_PORT, path: '/relay/s/' + s.code + req.url, method: req.method, headers },
    (pres) => {
...
wss.on('connection', (attach) => {
  ...
  function connectTunnel(s, path) {
    ...
    const url =
      'ws://' + API_HOST + ':' + API_PORT + '/relay/s/' + s.code + '/' + path + '?token=' + s.device_token;
